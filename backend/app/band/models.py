...
class Cover(models.Model):
    ...
    song: Song = ForeignKey(Song, related_name='covers',
                            on_delete=models.CASCADE)
    tags = ManyToManyField('CoverTag', db_table='Cover_Tags')
    likes = ManyToManyField(User, db_table='Cover_Likes',
                            related_name='like_covers')
    views: int = IntegerField(db_column='view', default=0)
    combination: 'Combination' = ForeignKey(
        'Combination', related_name='+', on_delete=models.SET_NULL, null=True)
...
class Combination(models.Model):
    ...
    song: Song = ForeignKey(
        Song, related_name='combinations', on_delete=models.CASCADE)
    covers = ManyToManyField(
        Cover, db_table='Cover_Combination', related_name='+')
    likes = ManyToManyField(
        User, db_table='Combination_Likes', related_name='like_combinations')
