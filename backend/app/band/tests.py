...
from .models import Instrument, Song, Cover, Combination


# pylint: disable=C0114
User = get_user_model()
instrument_name_list = ['bass guiter',
                        'acoustic guiter', 'vocal', 'drum', 'keyboard']
...
class BandModelTest(TestCase):
    ...
    def test_instruments(self):
        ...
        self.assertEqual(
            f'([{instrument.pk}] {instrument.name})', str(instrument))
    ...
    def test_song(self):
        ...
        self.assertEqual(
            f'([{random_song.pk}] {random_song.title})',
            str(random_song)
        )
